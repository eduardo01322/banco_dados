...
inner join funcionarios_projetos fp on fp.id_projeto = p.id_projeto 
...
-- Ex.3
select f.nome as funcionario, avg(horas_trabalhadas) as media from funcionarios_projetos fp 
inner join funcionarios f on f.id_funcionario = fp.id_funcionario 
group by f.nome;
...
-- Ex.5
select nome, dt_inicio from projetos p
order by dt_inicio 
limit 1;


-- Ex.6
update projetos set valor_total = 55000 
where id_projeto = 1;


-- Ex.7
select nome from projetos p
where dt_fim is null;


-- Ex.8
select avg(valor_total) as media from projetos p 
where status = 'em andamento';


-- Ex.9
select nome from funcionarios f 
where f.nome like '%1%'


-- Ex.10
select nome, valor_total from projetos p
...
-- Ex.12
select p.nome as projeto, f.nome as funcionario, sum(fp.horas_trabalhadas) from funcionarios_projetos fp 
inner join funcionarios f on f.id_funcionario = fp.id_funcionario 
inner join projetos p on p.id_projeto = fp.id_projeto
group by p.nome, f.nome
limit 5;


-- Ex.13
select f.nome, sum(fp.horas_trabalhadas) as total_horas from funcionarios f 
inner join funcionarios_projetos fp on fp.id_funcionario = f.id_funcionario
group by f.nome 
order by total_horas desc;

-- Ex.14
select p.nome as projeto, count(fp.id_funcionario) as total_horas from projetos p 
left join funcionarios_projetos fp on fp.id_projeto = p.id_projeto
group by p.nome;


-- Ex.15
select p.nome as projeto, f.nome as funcionarios, 
sum(fp.valor_hora * fp.horas_trabalhadas) as valor_pago 
from funcionarios f 
inner join funcionarios_projetos fp on fp.id_funcionario = f.id_funcionario
inner join projetos p on p.id_projeto = fp.id_projeto 
group by p.nome, f.nome; 
