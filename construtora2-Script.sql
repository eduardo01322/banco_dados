...
select * from projetos p where status = 'Em Andamento';


-- EX: 1  pt: 2
select nome, descricao, status from projetos_ativos pa;
	

-- Ex: 2 pt: 1
delimiter //
create procedure inserir_funcionario(nome varchar(255), email varchar(255), 
senha varchar(45), id_departamento int, valor_hora decimal(15,2))
begin
	insert into funcionarios (nome, email, senha, id_departamento, valor_hora) 
	values (nome, email, senha, id_departamento, valor_hora);
end //
delimiter ;

call inserir_funcionario('tttt', 't@gmail', '123456', 2, 2400.0);
...
-- Ex: 2 pt: 2
/* delimiter //
create procedure calcular_valor_total_projeto(horas_trabalhadas int, valor_hora decimal(15,2))
begin
	insert into funcionarios_projetos (horas_trabalhadas * valor_hora) 
	values (horas_trabalhadas * valor_hora);
end //
delimiter ;
	
call calcular_valor_total_projeto(2, 1000.00);
*/
...
begin
	declare horas int;
	select  sum(horas_trabalhadas) into horas from funcionarios_projetos p
	where p.id_projeto = id;
	return horas;
end //
DELIMITER ;

select calcular_horas_projeto(1) as horas;


-- EX: 3 pt: 2
DELIMITER //
create function calcular_custo_projeto()
returns decimal(15,2)
begin
	declare horas decimal(15,2);
	select  sum(horas_trabalhadas * valor_hora) into horas from funcionarios_projetos p;
	return horas;
end //
DELIMITER ;

select calcular_custo_projeto() as horas;
