...
/* O que é DCL?
DCL, ou Linguagem de Controle de Dados, é um subconjunto da SQL (Structured Query Language) 
que lida com os direitos de acesso e permissões em um banco de dados. 
Os dois comandos principais do DCL são:

GRANT - usado para dar permissões a usuários.
REVOKE - usado para retirar permissões de usuários.

Comando GRANT
O comando GRANT permite que você dê permissões a um usuário para realizar certas operações no banco de dados.
Sintaxe:
GRANT permissão1, permissão2, ... ON nome_do_banco_de_dados.nome_da_tabela TO 'nome_do_usuario'@'host';

Comando REVOKE
O comando REVOKE é usado para retirar permissões que foram anteriormente concedidas a um usuário.

Sintaxe:
REVOKE permissão1, permissão2, ... ON nome_do_banco_de_dados.nome_da_tabela FROM 'nome_do_usuario'@'host';

Permissões
ALL PRIVILEGES — permite ao usuário do MySQL acesso completo a um banco de dados (ou, se nenhum banco de dados for selecionado, acesso global a todo o sistema)
...
create user 'pizzaria'@'localhost' identified by '123';
grant all privileges on pizzaria.* to 'pizzaria'@'localhost';
show grants for 'pizzaria'@'localhost';


create user 'cliente'@'localhost' identified by '123';
grant select on pizzaria.pizza to 'cliente'@'localhost';
grant select, insert, update, delete on pizzaria.pedido to 'cliente'@'localhost';
grant insert on pizzaria.item_pedido to 'cliente'@'localhost';
show grants for 'cliente'@'localhost';


revoke select on pizzaria.pedido from 'cliente'@'localhost';
